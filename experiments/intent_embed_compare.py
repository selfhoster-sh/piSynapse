...
def main():
    ...
    rss_after_mini = _rss_mb()

    t0 = time.perf_counter(); m_e5 = load(E5); tl_e5 = time.perf_counter()-t0
    ...
    t0=time.perf_counter(); vecs(m_mini, speed_texts); sp_mini=time.perf_counter()-t0
    t0=time.perf_counter(); vecs(m_e5,  speed_texts); sp_e5 =time.perf_counter()-t0
    ...
    for msg, expected in CASES:
        ...
        kw = _keyword_group(msg) if False else None  # keyword ayrı raporlanacak
    ...
    import statistics
    mm = [r[3] for r in rows]; me5 = [r[7] for r in rows]
    thin_m = sum(1 for x in mm if x < 0.10); thin_e = sum(1 for x in me5 if x < 0.10)
    acc_m = sum(1 for r in rows if r[4] and r[5]); acc_e = sum(1 for r in rows if r[8] and r[9])
