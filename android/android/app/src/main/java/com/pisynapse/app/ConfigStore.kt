...
class ConfigStore(private val ctx: Context) {
    ...
    private fun defaults(): JSONObject = JSONObject()
        ...
        .put("LLM_HISTORY_LIMIT", 40)
        // Rolling summary contract (server-parity: piServe SUMMARY_BATCH_SIZE / SUMMARY_EARLY_TRIGGER)
        .put("SUMMARY_BATCH_SIZE", 5)
        .put("SUMMARY_EARLY_TRIGGER", 6)
    ...
    fun schema(): JSONObject {
        ...
        add("LLM_HISTORY_LIMIT", v("LLM_HISTORY_LIMIT"), "number", "Geçmiş yükleme (mesaj)", "Conversation yeniden kurulurken geri yüklenen son mesaj sayısı (ne kadar çok, o kadar uzun prefill).", "Telefon · Üretim")
        add("SUMMARY_BATCH_SIZE", v("SUMMARY_BATCH_SIZE"), "number", "Özet parti (mesaj)", "Her özet adımında katlanan mesaj sayısı — piServe SUMMARY_BATCH_SIZE ile aynı sözleşme.", "Telefon · Sohbet")
        add("SUMMARY_EARLY_TRIGGER", v("SUMMARY_EARLY_TRIGGER"), "number", "Özet erkenden tetikle", "İlk özet, bu kadar mesaj birikince ateşlenir (piServe SUMMARY_EARLY_TRIGGER ile aynı).", "Telefon · Sohbet")
