...
- **Applied (`820c688`):** relief flip is now INSTANT (transition removed) and the relief tints hold the at-rest FROST tone — topbar accent 7% + `rgba(14,14,20,.78)`, input `rgba(14,15,21,.85)`, sheen gradient kept (`background-color`, not `background`). Mobile input keeps its exact at-rest `.88` tint during relief via a later-in-file `@media (hover:none)` override (same specificity → source order wins). Sidebar dropped from the relief selector.
- **Cheaper at-rest blur:** pinned chrome blur 12→**8px** desktop (`blur(8px) saturate(140%)`), mobile topbar 12→8, mobile input 16→12. Sidebar `backdrop-filter:none` (its backdrop is the fixed ambient aurora — never scrolls → re-blur is pure waste; sheen gradient + translucent `--surface` keep the look). Desktop input base now `rgba(13,13,18,.55)` (at-rest), so the relief delta is small.
...
- **Push state (current):** `origin/main` at `ffb63c3`; unpushed local: `b9a16c5` (tool indicator), `e4fc7bc` (journal C-5), and this item.
