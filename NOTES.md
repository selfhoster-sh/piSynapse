...
| **8** | **Avahi fix** | `/etc/avahi/avahi-daemon.conf` → `allow-interfaces=eth0`. `.local` çözümlemesi Docker bridge (`172.17.0.1`) yerine gerçek IP (`192.168.1.109`) döndürüyor. |
...
**Sorun:** LAN üzerinden (`http://10.9.9.3:8765`) erişildiğinde tarayıcı `getUserMedia` API'sini bloke eder çünkü HTTP + non-localhost origin "güvenli olmayan bağlam" (insecure context) sayılır. Sonuç: sesli konuşma (mikrofon) çalışmaz. `localhost` veya `127.0.0.1`'den erişimde sorun yok.

**Çözüm A — VPS üzerinden NPM proxy (önerilen):**
Mevcut Nginx Proxy Manager'a (VPS'te, Docker `npm-app-1`) yeni bir proxy host eklenir:
- Domain: `pi.sudosalih.com`
- Forward: `10.9.9.3:8765` (Pi5'in AWG IP'si)
- SSL: Mevcut Let's Encrypt sertifikası (`*.sudosalih.com` wildcard veya yeni cert)
- Telefon `https://pi.sudosalih.com` ile erişir → VPS 443 → AWG tunnel → Pi5:8765
...
- `pi.sudosalih.com` DNS kaydı Pi5 LAN IP'sini (veya AWG IP'sini) gösterecek şekilde ayarlanır
...
- `mkcert 10.9.9.3 192.168.x.x localhost` ile cert imzalanır
