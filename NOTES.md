...
- **Task 1 — secrets (user: no rotation, gitignore suffices, my discretion):** verified `backups/`, `.env`, `*.db*` all gitignored; all archives already 0600 (Faz 0b chmod); no rotation performed per decision. Nothing to commit.
...
- **Faz 3d:** `_enrich_title` first-chance (total≤4) + never-overwrite (unset/"New Chat"/RAKE only) using the FIRST turn (get_history is chronological ASC — an initial `[::-1]` bug caught by the new test); RAKE masks emails/URLs; migration runner quotes identifiers + rejects DDL metacharacters.
...
## 2026-09-06 — Faz ONNX-EMBEDDING: yerel multiplikual vektörizasyon (telefonda 7 nokta doğrulandı)
...
## 2026-09-05 — Faz ANDROID-VALIDATE: ayarlar 2 sekme + izinler + hafıza native uçtan uca doğrulandı
