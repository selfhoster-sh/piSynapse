...
- **Open (MADDE 2):** live repro of the single-group narrowing depends on the model emitting the marker; rare/non-deterministic — covered by unit test (`test_escalation_tools_single_group`). Live enforcement verified instead against `intent_audit_log`: **46/46** `question+None` rows have EMPTY `_hit_groups` (no keyword), so `hint_armed=True` never occurs in production — keyword-bearing messages all route to `action`; this is direct evidence the (former) spam escalation root cause is gone. Full accounting in `BUG_RAFLAMA_DURUM.md` §8.
- **Embedding carryover (MADDE 3, closed):** 10 posts remained at the old **384-dim** `conversations.embedding` (1536-byte BLOBs; timestamps all 2026-09-02 22:11–22:45, so they pre-dated the migration). Ran the standard `reembed_all.py` → all 216 conversation + 3 memory rows re-embeded at the configured **768-dim** (`paraphrase-multilingual-mpnet-base-v2`); verified `SELECT length(embedding) GROUP BY`: 216×3072 + 3×3072, **0 rows at 1536**. DB and `.npy` are gitignored, so no code commit required. Note for future docs: `_get_tool_embeddings` (llm/intent.py:460) does NOT read `additions_embeddings.npy` anymore — it embeds `_TOOL_EMBED_CORPUS + _additional_corpus()` **in-memory** (67 entries) and caches them; deleting the `.npy` is harmless (witnessed live).
