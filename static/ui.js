...
function renderMd(raw){
  ...
  // Fallback sanitizes too: if marked ever fails to load, raw markdown must
  // never reach innerHTML unsanitized (renders as plain text instead).
  let r = window.marked && window.marked.parse ? sanitizeHTML(marked.parse(s)) : sanitizeHTML(s);
...
function renderImagePreview(){
  ...
  box.innerHTML = pendingImages.map((img,i) =>
    `<div class="preview-item"><img src="${esc(img.preview)}" alt="${esc(img.name)}"><button class="preview-remove" onclick="removeImage(${i})">&times;</button></div>`
  ).join('');
}
...
function renderOnboarding(){
  ...
  obRenderDots();
  // The gate/login/ceremony steps hide Next; every other branch re-shows it
  // explicitly — but reset here so no path can ever strand the wizard.
  ...
  if(step === 's1'){
    ...
    body.innerHTML = `
      ...
      <details style="margin-top:12px"><summary style="cursor:pointer;color:var(--accent);font-size:13px">${esc(t('curiousT'))}</summary>
        <div class="ob-row-sub" style="margin-top:8px">${esc(t('cxWhat'))}<br><br>${esc(t('cxModel'))}<br><br>${esc(t('cxTools'))}<br><br>${esc(t('cxWeather'))}<br><br>${esc(t('cxThumbs'))}<br><br>${esc(t('cxLoop'))}<br><br><em>${esc(t('tourSpeed'))}</em></div>
      </details>`;
    next.dataset.action = '';
