...
| `MAIL_PROVIDER` | `gmail`, `proton`, or leave empty | `gmail` |
...
curl http://localhost:8000/chat/history?session_id=main \
  -H "X-API-Key: YOUR_API_KEY"
...
# Delete all memories
curl -X DELETE "http://localhost:8000/chat/memories?user_id=default" \
  -H "X-API-Key: YOUR_API_KEY"
