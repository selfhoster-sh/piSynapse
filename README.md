...
- The LLM model (Gemma4 E2B ~2.4 GB or E4B ~3.4 GB via piServe/LiteRT) is loaded into RAM and stays resident while the server is running. With Ollama the same models may use more RAM due to different quantization. On 8 GB devices, budget accordingly.
...
| `LITERT_BASE_URL` | LiteRT/piServe server URL | `http://localhost:9379` |
...
**piServe:** When `LLM_BACKEND=litert`, piServe serves at `LITERT_BASE_URL` (`:9379`) and is configured via `litert_serve/config.json` (model, context window, MTP). The installer generates it automatically.
