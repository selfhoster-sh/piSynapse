...
### Fixed
- Single source of truth for config: `EMBED_MODEL` canonicalized to mpnet-base-v2 everywhere (mismatched defaults silently zeroed similarity scores); new startup warning on embedding dimension drift. NOTE: installs without an explicit `EMBED_MODEL` key that stored MiniLM (384-dim) vectors will see empty retrieval until `reembed_all.py` runs — the startup warning says so.
...
- Chat parity: non-stream path now arms the tool hint, escalates, and honors chip fast-path exactly like streaming (shared helpers); mailbox search fixed; listings numbered; LiteRT keeps tool names; indexless deltas accumulate; trim keeps tool triples atomic; broken tool JSON no longer collides on `{}`; "Durdur" stops background tools with a terminal event; group tool lists derive from `TOOL_GROUPS`; calendar/task/note outputs sanitized; RRULE allowlisted; email addresses validated; verification failures visible; retrieval dedupes verbatim overlap; CalDAV reads and reload drain bounded; one shared weather format.
- Hardening & ops: no wildcard trust bypass; exempt paths rate-limited; no URL-key auth; redacted debug; masked settings secrets; upload type/magic gates; audio suffix allowlist; repo-anchored paths; DB permission sweep; installer log rotation; pinned lockfile; degraded boot with DB health signal; automatic owner-only DB backups (keep 7) + restore guide; audit-CSV retention; paged history export.
- Installer: batch mode works end-to-end; atomic voice downloads; rerun-honest env handling; TTS skip honesty; pre-flight resource checks with optional model pre-download; download retries.
